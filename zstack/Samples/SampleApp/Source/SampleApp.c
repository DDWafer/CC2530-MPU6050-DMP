...
uint8 end_flag = 0;
uint16 end_count = 0;
extern uint16 count;             //用于定时器计数
extern uint8 timesleep_count;
extern uint8 LedState;
extern uint8 open_flag;  //开机信号 
extern uint8 close_flag;  //关机信号
extern uint8 timesleep;   //定时休眠 
extern uint8 untimesleep;//无休止休眠
 ...
 * LOCAL VARIABLES
 */
uint8 send_flag=0;//用于自动发送关闭信号后的延时
 ...
 * @return  none
 */
extern void ADXL345_WR_Reg(unsigned char addr, unsigned char val);//加速度传感器写函数
extern unsigned char ADXL345_RD_Reg(unsigned char addr);//加速度传感器读函数
extern float Pitch;
extern float Roll;
extern float Yaw;
void SampleApp_SendPeriodicMessage( void )
...
//  temp[3] = ADXL345_RD_Reg(0x20);
  ...
  //HalUARTWrite(0,&temp[0], sizeof(temp[0]));//提示信息
...
//    I2C_Read(0xd0,0x40, 1, &test_data[5]);
    ...
    if(count==2)
    {
      //HalUARTWrite(0,&(temp_p), sizeof(temp_p));//提示信息
      //HalUARTWrite(0,&(temp_r), sizeof(temp_p));//提示信息
      //HalUARTWrite(0,&(temp_y), sizeof(temp_p));//提示信息
